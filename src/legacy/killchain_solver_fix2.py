...
import pulp
import numpy as np
# matplotlib는 그리기 전용 — 없으면 자동 생략 (build_and_solve는 미사용)
try:
    import matplotlib
    matplotlib.use("Agg")
...
def build_and_solve(xls_path: str, time_limit: int = 120, msg: bool = True):
    ...
    # Tightened Big-M: 노드/측면별 타이트 상한 (기존 (max_t_rec+max_t_atk)*10 은
    #   LP relaxation 붕괴 → BF 유발). 정찰측 tau/D 는 정찰 완료시각 상한으로,
    #   공격측 T/시간창 은 (정찰완료 + 공격이동) 상한으로 충분.
    max_t_rec = max(r_fuel[r] / r_speed[r] for r in R)
    max_t_atk = max(a_fuel[a] / a_speed[a] for a in A)
    max_arc_rec = max((t_rec[(r,i,j)] for (r,i,j) in t_rec), default=0.0)
    max_arc_atk = max((t_atk[(a,i,j)] for (a,i,j) in t_atk), default=0.0)
    M_REC = max_t_rec + max_arc_rec                  # 정찰측 (tau, D)
    M_ATK = max_t_rec + max_t_atk + max_arc_atk      # 공격측 (T, 시간창)
    ...
    for r in R:
        ...
        for p in P:
            prob += (tau[(r,p)] >= t_rec[(r,dep,p)] - M_REC*(1 - x_rec[(r,dep,p)])),\
                    f"tau_depot_{r}_{p}"
        for p in P:
            for q in P:
                if p == q: continue
                prob += (tau[(r,q)] >= tau[(r,p)] + t_rec[(r,p,q)]
                         - M_REC*(1 - x_rec[(r,p,q)])), f"tau_prop_{r}_{p}_{q}"
    ...
    for p in P:
        prob += D[p] <= M_REC * pulp.lpSum(z[(r,p)] for r in R), f"D_upper_{p}"  # (iii)
        for r in R:
            prob += D[p] <= tau[(r,p)] + M_REC*(1 - z[(r,p)]), f"D_le_{r}_{p}"  # (ii)
            prob += D[p] >= tau[(r,p)] - M_REC*(1 - z[(r,p)]), f"D_ge_{r}_{p}"  # (i)
        ...
        for i in Ka:
            for j in Ka:
                ...
                prob += (T[(a,j)] >= D[pk_j] + t_atk[(a,i,j)]
                         - M_ATK*(1 - y_atk[(a,i,j)])), f"T_prop_{a}_{i}_{j}"

        # (B-6) 정찰 선행 조건 + 타임윈도우
        for k in Ka:
            pk = k_point[k]
            prob += h[(a,k)] <= pulp.lpSum(z[(r,pk)] for r in R), f"activate_{a}_{k}"
            prob += T[(a,k)] >= D[pk] - M_ATK*(1 - h[(a,k)]),    f"tw_lo_{a}_{k}"
            prob += T[(a,k)] <= D[pk] + k_win[k] + M_ATK*(1 - h[(a,k)]), f"tw_hi_{a}_{k}"
    ...
    # ── (전처리) 도달 불가 표적 사전 고정 ─────────────────────────
    # feasibility 필요조건: T[a,k] = D[p_k] + (선행→k 이동), 시간창 상한 = D[p_k] + Δk
    #   ⟹ 최소 진입 이동 ≤ Δk 이어야 타격 가능. depot/동일구역 타표적에서의
    #   최소 이동조차 Δk 초과면 h=0 고정 (d_{0k}/v_A ≤ Δk 의 일반화; 증명상 안전).
    n_prefixed = 0
    for a in A:
        dep = atk_depot_id(a)
        Ka  = K_a[a]
        for k in Ka:
            cand = [t_atk[(a,dep,k)]] + [t_atk[(a,i,k)] for i in Ka if i != k]
            if min(cand) > k_win[k] + 1e-9:
                prob += h[(a,k)] == 0, f"prefix_unreach_{a}_{k}"
                n_prefixed += 1
    if msg:
        print(f"  [tight] pre-fixed unreachable targets: {n_prefixed}")

    # ── 2-phase: 정찰 전지점 커버 NN투어로 고정 → 공격만 최적화 ──────
    #   구조적 사실: 타격 feasibility 조건에서 D[p_k] 가 상쇄되어 정찰 *타이밍*
    #   은 타격 가능성과 무관. 항속거리 충분 → 최적 커버리지 = 전 지점.
    #   ∴ 정찰을 전지점 커버 투어(구역별 UAV 1대)로 고정해도 결합 최적해 불변.
    #   거대 정찰 TOP(분기 폭발·obj=0 함정) 완전 우회.
    zones_sorted = sorted(set(p_zone.values()))
    zone_to_uav = {zv: R[i % len(R)] for i, zv in enumerate(zones_sorted)}
    uav_pts = {r: [p for p in P if zone_to_uav[p_zone[p]] == r] for r in R}
    def _fix(var, val):
        var.lowBound = val; var.upBound = val; var.setInitialValue(val)
    cover = 0
    for r in R:
        dep = rec_depot_id(r)
        # 전부 0 고정 후 투어만 1
        for p in P:
            _fix(z[(r,p)], 0); _fix(u[(r,p)], 0)
        for (rr,i,j) in list(x_rec):
            if rr == r: _fix(x_rec[(r,i,j)], 0)
        cur = dep; remaining = set(uav_pts[r]); order = []; total = 0.0
        while remaining:
            nxt = min(remaining, key=lambda q: d_rec[(r,cur,q)])
            if total + d_rec[(r,cur,nxt)] + d_rec[(r,nxt,dep)] > r_fuel[r]:
                break
            total += d_rec[(r,cur,nxt)]; order.append(nxt); remaining.discard(nxt); cur = nxt
        prev = dep
        for idx, p in enumerate(order, start=1):
            _fix(x_rec[(r,prev,p)], 1); _fix(z[(r,p)], 1); _fix(u[(r,p)], idx)
            prev = p
        if order:
            _fix(x_rec[(r,prev,dep)], 1)
        cover += len(order)
        if remaining and msg:
            print(f"  [fix2] WARN uav {r}: {len(remaining)} pts uncovered (range)")
    if msg:
        print(f"  [fix2] recon fixed, covered {cover}/{len(P)} points")
