...
│   ├── run_extension.py       Extension validation at a single N_P level
...
│   ├── extension_np12.csv     Extension validation at N_P = 12 (all certified)
│   ├── extension_np14.csv     Extension attempt at N_P = 14 (partly uncertified)
...
│       └── provenance_2026-03/  Raw output of earlier model versions
...
The structural ceiling is confirmed outside the factor range of the main design by
re-running the four factorial corners at a higher target density, without altering the
main design itself:

```bash
python run_extension.py --np 12 --out ../results --threads 16
```

At `N_P = 12` the weapon-to-target ratio is `8 / 12 = 0.6667`. All twenty-four runs certify;
the eighteen at corners with either temporal factor high return a hit ratio of `0.666667`
with zero variance. Output uses the same schema as `exp*_raw.csv`.

`results/extension_np14.csv` records an attempt at a further level that was abandoned. It is
retained deliberately: two of its three high-speed narrow-window runs failed to produce a
feasible solution for one zone within a 1,800-second-per-subproblem budget on sixteen
threads, so their `proven` flag is `False` and their strike values are incumbents rather than
optima. **Rows with `proven == False` must be excluded from any analysis.** `make_figures.py`
only folds an extension file into Figure 4 when every row in it is proven.
...
`results/legacy/provenance_2026-03/` holds the raw output of earlier model versions,
including the reconnaissance-count screening design whose results were withdrawn, with a
README explaining what each file shows and why it was superseded. None of it feeds the
reported results; it is retained so that the development history is auditable.
...
[`CITATION.cff`](CITATION.cff). The DOI below is the **concept DOI**: it always resolves to
the most recent release. Cite a specific release only if you need to pin an exact version.
